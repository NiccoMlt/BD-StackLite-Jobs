...
\subsection[%
  ...
  \begin{itemize}
    ...
    \item \textbf{Conteggio} : L'informazione contenuta nel Data Frame \texttt{binDF} viene arricchita dall'aggiunta
    ...
    \texttt{SELECT Tag, Bin, COUNT(*) AS Count FROM binDF GROUP BY Tag, Bin}
    ...
    \texttt{SELECT Bin, COLLECT\_LIST(STRUCT(Tag, Count)) AS ListTagCount FROM binCountDF GROUP BY Bin}

    \item \textbf{Ordinamento ed estrazione top ten} : A partire dal Data Frame dello step precedente, viene effettuato un
